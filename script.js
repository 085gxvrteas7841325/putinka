...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', function() {
            ...
            console.log(`Found ${allCards.length} method cards for searching.`);
            ...
            allCards.forEach((card, index) => {
                ...
                console.log(`Card ${index}: Title='${title.substring(0,30)}...', Content='${content.substring(0, 30)}...'`);

                const matchesQuery = !query || title.includes(query) || content.includes(query);
                console.log(`Card ${index} ('${title.substring(0,15)}...'): Matches query '${query}'? ${matchesQuery}`);
